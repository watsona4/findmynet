...
TOPIC="testmynet/healthcheck/status"
...
if ! pgrep -f "mosquitto_sub.*$TOPIC" > /dev/null; then
    ...
    mosquitto_sub -h "$BROKER" -t "$TOPIC" > /tmp/testmynet_healthcheck_response.txt &
...
sleep "$TIMEOUT"
RESPONSE=$(tail -n 1 /tmp/testmynet_healthcheck_response.txt)
